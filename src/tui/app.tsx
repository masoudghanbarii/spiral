...
export function TuiApp({
  ...
  const systemPrompt = useMemo(() => {
    ...
    const modeName = activeSession.mode || "agent";
    return `You are Spiral, an AI co-founder and development agent. You are currently running in ${modeName} mode.\n\nYou help developers build software. You have tools to read/write files, search code, run commands, and manage git.\nYou also have tools to communicate with other sessions.\n\n## When to use tools\n\nDO NOT use tools for:\n- Greetings, small talk, or casual conversation\n- Questions about yourself, your mode, or how you work\n- General knowledge questions\n- Anything that doesn't require reading files or running commands\n\nDO use tools for:\n- Implementing features, fixing bugs, writing code\n- Reading, searching, or modifying the project\n- Running commands, tests, or git operations\n- The user explicitly asks you to do something with the codebase\n\n## Session Tools\n\nWhen the user asks to pass info to another session, ALWAYS do both:\n1. send_to_session to deliver the message:\n   {"target_session": "s2", "message": "the info to pass"}\n2. link_sessions to visually group the sessions:\n   {"session_a": "s1", "session_b": "s2"}\n\nAlways pass tool arguments as valid JSON with the exact parameter names shown.\n\nThink step by step. When you DO use tools, wait for results before proceeding.${suffix}`;
  }, [activeSession.mode]);
  ...
  useEffect(() => {
    ...
    // Ctrl+Backspace in many terminals sends 0x08 (BS) — we intercept it
    // here because Ink can't distinguish it from regular Backspace (0x7f).
    // Also handle Alt+Backspace (\x1b\x7f).
    ...
    const onData = (data: Buffer) => {
      ...
      // Check for word-delete sequences (Ctrl+Backspace or Alt+Backspace)
      if (pending === CTRL_BACKSPACE || pending === ALT_BACKSPACE) {
        wordDelete();
        pending = "";
        return;
      }
  ...
  useInput((inputChar: string, key: any) => {
    ...
    if ((key.backspace || key.delete) && !key.ctrl && !key.meta) {
      setInput((prev) => prev.slice(0, -1));
      return;
    }

    // ── Word deletion: Ctrl+W, Alt+Backspace, or Ctrl+Backspace ──
    // In most Linux terminals, Ctrl+Backspace sends 0x7f (same as regular
    // Backspace), so it's indistinguishable. We support three shortcuts:
    //   - Ctrl+W       (reliably detected by Ink)
    //   - Alt+Backspace (Ink sets key.meta on backspace)
    //   - Ctrl+H        (0x08, detected in raw stdin listener)
    if ((key.ctrl && inputChar === "w") || (key.meta && (key.backspace || key.delete)) || (key.ctrl && inputChar === "h")) {
      setInput((prev) => {
